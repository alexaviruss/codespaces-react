...
import { ArrowUpRight, ArrowDownLeft, CheckCircle, Trash2, Plus, User, Calendar, FileText, IndianRupee } from "lucide-react";
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
      ...
      <main className="px-4 py-4 max-w-md mx-auto space-y-4">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-teal-600 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-teal-700 transition-all"
        >
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="space-y-3 rounded-2xl bg-white p-4 shadow-sm border border-slate-100">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm text-slate-900 focus:border-teal-600 focus:outline-none"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2 pl-9 pr-2 text-xs text-slate-900 focus:border-teal-600 focus:outline-none"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-slate-900 py-2.5 text-xs font-semibold text-white hover:bg-slate-800 transition-all"
            >
        ...
        {udhaariList.length === 0 ? (
          <div className="rounded-2xl bg-white p-8 text-center shadow-sm border border-slate-100">
            <p className="text-sm text-slate-400">No lending or borrowing records found.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {udhaariList.map((item) => (
              <div
                key={item.id}
                className={`flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm border transition-all ${
                  item.status === "Settled" ? "opacity-50 border-slate-100" : "border-slate-200"
                }`}
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-slate-800">{item.personName}</h3>
                  </div>
